...
(function() {
  ...
  decorate = function(api, md, slugCache, hideUri, demoPath, verbose) {
    var action, category, dataStructure, dataStructures, err, example, i, item, j, k, knownParams, l, len, len1, len2, len3, meta, name, newParams, param, ref, ref1, ref2, ref3, resource, resourceGroup, results, reversed, schema, slugify, templateUri;
        ...
        for (m = 0, len4 = ref4.length; m < len4; m++) {
          ...
          results1.push((function() {
            ...
            for (n = 0, len5 = ref5.length; n < len5; n++) {
              ...
              templateUri = (action.attributes || {}).uriTemplate || resource.uriTemplate || '';
              if (hideUri) {
                action.uriTemplate = '';
                action.colorizedUriTemplate = '';
              } else {
                action.uriTemplate = modifyUriTemplate(templateUri, action.parameters);
                action.colorizedUriTemplate = modifyUriTemplate(templateUri, action.parameters, true);
              }
              if (templateUri.indexOf('ignore') !== -1) {
                action.phpDemoUri = '';
                action.javaDemoUri = '';
              } else {
                action.phpDemoUri = demoPath + 'php/prod' + modifyUriTemplate(templateUri, action.parameters) + '/index.php';
                action.javaDemoUri = demoPath + 'java/prod' + modifyUriTemplate(templateUri, action.parameters) + '/index.jsp';
              }
      ...
      options: [
        {
          ...
          name: 'hide-uri',
          description: 'Whether to hide the URI for each resource',
          boolean: true,
          "default": false
        }, {
          name: 'skip-schema',
          description: 'Whether to skip schema section',
          boolean: true,
          "default": false
        }, {
          name: 'demo-path',
          description: 'The path prefix for the demo',
          "default": 'demo'
        }, {
          name: 'template',
          ...
          name: 'navigation',
          description: 'Navigation items on top of the page',
          "default": 'default'
        }
      ]
    };
  ...
  exports.render = function(input, options, done) {
    var i, len, linkPattern, md, navItem, navItems, slugCache, themeStyle, themeVariables, values, verbose;
    if (done == null) {
      done = options;
      options = {};
    }
    ...
    if (options.themeHideUri == null) {
      options.themeHideUri = false;
    }
    if (options.themeSkipSchema == null) {
      options.themeSkipSchema = false;
    }
    if (options.themeDemoPath == null) {
      options.themeDemoPath = 'demo';
    }
    if (options.themeNavigation == null) {
      options.themeNavigation = '';
    }
    if (options.themeNavigation !== 'default') {
    ...
    decorate(input, md, slugCache, options.themeHideUri, options.themeDemoPath, options.verbose);
