...
decorate = (api, md, slugCache, hideUri, demoPath, verbose) ->
  ...
  for resourceGroup in api.resourceGroups or []
    ...
    for resource in resourceGroup.resources or []
      ...
      for action in resource.actions or []
        ...
        templateUri = (action.attributes or {}).uriTemplate or
          resource.uriTemplate or ''
        if hideUri
          action.uriTemplate = ''
          action.colorizedUriTemplate = ''
        else
          action.uriTemplate = modifyUriTemplate(templateUri, action.parameters)
          action.colorizedUriTemplate = modifyUriTemplate(
            templateUri, action.parameters, true)

        if templateUri.indexOf('ignore') != -1
          action.phpDemoUri = ''
          action.javaDemoUri = ''
        else
          action.phpDemoUri = demoPath + 'php/prod' +
            modifyUriTemplate(templateUri, action.parameters) +
            '/index.php'
          action.javaDemoUri = demoPath + 'java/prod' +
            modifyUriTemplate(templateUri, action.parameters) +
            '/index.jsp'
...
exports.getConfig = ->
  ...
  options: [
    ...
    {name: 'hide-uri', description: 'Whether to hide the URI for each resource',
    boolean: true, default: false},
    {name: 'skip-schema', description: 'Whether to skip schema section',
    boolean: true, default: false},
    {name: 'demo-path', description: 'The path prefix for the demo',
    default: 'demo'},
    ...
    boolean: true, default: true},
    {name: 'navigation', description: 'Navigation items on top of the page',
    default: 'default'}
  ]
...
exports.render = (input, options, done) ->
  ...
  options.themeHideUri ?= false
  options.themeSkipSchema ?= false
  options.themeDemoPath ?= 'demo'
  options.themeNavigation ?= ''

  if options.themeNavigation isnt 'default'
    navItems = options.themeNavigation.split "|"
    options.themeNavigation = {}
    for navItem in navItems
      values = navItem.split "="
      options.themeNavigation[values[0]] = values[1]
  else
    options.themeNavigation = null
  ...
  md = markdownIt(
    ...
    slugify: (value) ->
      matches = value.match linkPattern
      if matches
        slugCache._nav.push [matches[1], matches[2], "_blank"]
        return matches[2]
      else
  ...
  decorate input, md, slugCache, options.themeHideUri,
    options.themeDemoPath, options.verbose
  ...
  getCss themeVariables, themeStyle, verbose, (err, css) ->
    ...
    locals =
      ...
      hideUri: options.themeHideUri
      skipSchema: options.themeSkipSchema
      navigation: options.themeNavigation
      date: moment
      hash: (value) ->
